...
const setLayerTransforms = async (transforms, scale, rotation) => {
    ...
    const sets = transforms.flatMap(t => {
        ///it turns out getting the previous transform amount was pretty pointless because these transforms are already
        //additive to whatever it was, so 100% here will keep things the same and i don't need to compute a new scale percentage
        //based on the existing value. what a waste of time. At some point i can remove that step altogether although it still
        //may be needed for non-smart objects. 
        return [
            {
                _obj: 'select',
    ...
    console.log("(setLayerTransforms) Sets:", sets);
...
const processSmartObjectLayers = async (layers, scale, rotation) => {
    ...
    console.log("(processSmartObjectLayers) Transforms:", transforms);
