...
async function getKitchenSink(layer) {
    ...
    if (layer.kind === constants.LayerKind.SOLIDFILL) {
        try {
            ...
            results[0].pathContents = vectorInfo[0];
        } catch (error) {
...
const setVectorInfo = async (layer) => {
    ...
    const results1 = await runBp(command);
    ...
    const results2 = await runBp(command2);
    ...
    const results3 = await runBp(command3);
...
export { getKitchenSink, getVectorInfo, setVectorInfo };