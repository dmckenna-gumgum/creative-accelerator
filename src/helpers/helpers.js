...
function findMatchingLayerNames(searchScope, targetNames, matchingLayers = []) {
    for (const layer of searchScope) {
        if (targetNames.has(layer.name)) {
            // layer.selected = true;
            //turning this off, it's faster to batchplay the selection action afterwards.
        ...
        if (layer.layers) {
            findMatchingLayerNames(layer.layers, targetNames, matchingLayers);
        }
    }
...
async function showInputDialog(label, title, defaultValue = '', okText = 'OK', cancelText = 'Cancel') {
    return new Promise(async (resolve) => { // Make the promise callback async
        try {
            ...
            if (!htmlFile) {
                console.log("Dialog HTML file not found: html/inputDialog.html");
