...
function createActionPlans({ buildStep, activeDoc, creativeState, createdBoards, subStep = false }) {
    const plans = [];
    const { devices, sourceSequences, sourceStep, destinationSteps } = subStep ? buildStep.stepActionScope : buildStep.advanceActionScope;
    devices.forEach(deviceName => {
        ...
        sourceSequences.forEach(sequenceName => {
            ...
            if (sourceBoard) {
                ...
                if (!existingSource) creativeState.devices[deviceName].sequences[sequenceName].artboards.push(artboardEntry);
            }
        });
    });
...
async function propagateToNewIntroSequence(buildStep, creativeConfig, subStep = false) {
    const context = {
...
async function executeAllActionPlans(actionPlan, context) {
    ...
    for (const plan of actionPlan) {
        const renamedBoards = plan.destinationSequences.flatMap(destinationSequence => renamePreviousBoards(context.creativeState.devices[plan.deviceName].sequences[destinationSequence]));
        for (let i = 0; i < plan.destinations.length; i++) {
            ...
            context.creativeState.devices[plan.deviceName].sequences[plan.sequenceName].artboards.push(artboardEntry);
